Date.prototype.addDaysDatePicker = function (days) {
    this.setDate(this.getDate() + parseInt(days));
    return this;
};


function getDatePicker(days) {
    var today = new Date().addDaysDatePicker(days);
    ...
    $.fn.daterangepicker = function (settings) {
        ...
        var options = $.extend({
            presetRanges: [
                ...
                { text: 'Today', dateStart: getDatePicker(0), dateEnd: getDatePicker(0) },
                { text: 'Last 7 days', dateStart: getDatePicker(-7), dateEnd: getDatePicker(0) },
                { text: 'Month to date', dateStart: function () { return Date.parse(getDatePicker(0)).moveToFirstDayOfMonth(); }, dateEnd: getDatePicker(0) },
                { text: 'Year to date', dateStart: function () { var x = Date.parse(getDatePicker(0)); x.setMonth(0); x.setDate(1); return x; }, dateEnd: getDatePicker(0) },
                //extras:
                { text: 'The previous Month', dateStart: function () { return Date.parse(getDatePicker(-31)).moveToFirstDayOfMonth(); }, dateEnd: function () { return Date.parse(getDatePicker(-31)).moveToLastDayOfMonth(); } }
            ...
            target: rangeInput,
            doneButtonText: 'Done',
            earliestDate: Date.parse('01/01/1960'), //earliest date allowed 
            ...
            onClose: function () { },
            onOpen: function () { },
            onChange: function () { ChangeFunctionForRangePicker(rangeInput); },
            datepickerOptions: null //object containing native UI datepicker API options
        }, settings);


        function ChangeFunctionForRangePicker(rangeInput) {

            var operatorSelectBoxID = $(rangeInput).attr('id'); //Finding the ID of the range input.
            var dataInput = $(rangeInput).val();
            //Here we go. The data attribute is used in the advanced search textboxes.


            operatorSelectBoxID = $(rangeInput).attr("data");
            //If the data attribute isn't defined, go ahead and use a different (non-advanced) naming convention.
            if (typeof operatorSelectBoxID == 'undefined') {
                operatorSelectBoxID = $(rangeInput).attr('id');
                operatorSelectBoxID = operatorSelectBoxID.concat("_operator"); //adding _operator to that ID.
            }
            //Find the select box after appending a # to the name.
            operatorSelectBoxID = "#" + operatorSelectBoxID;
            var operatorSelectBox = $(operatorSelectBoxID); //Finding the select box.
            //Check data value and go ahead and see if it's a between or not. Then set it
            if (dataInput.indexOf(":") == -1) {
                if (pickerOption == 1) {
                    operatorSelectBox.val('Greater Than');
                } else {
                    operatorSelectBox.val('Equals');
                }

            } else {
                operatorSelectBox.val('Between');
            }
        ...
        var datepickerOptions = {
            onSelect: function (dateText, inst) {
                ...
                if (rp.find('.ui-daterangepicker-allDatesAfter').is('.ui-state-active')) {
                    range_end.datepicker('setDate', range_start.datepicker('getDate'));
                }
        ...
        function positionRP() {
            ...
            //Added 8/18/17. This will add an offset if the dropdown won't fit on the page.
            if ($(window).height() < riOffset.top + 300 + relEl.outerHeight()) { rp.parent().addClass('upsideDown'); } else { rp.parent().removeClass('upsideDown'); }
        }
        ...
        function clickActions(el, rp, rpPickers, doneBtn) {
            ...
            if (el.is('.ui-daterangepicker-specificDate')) {
                ...
                rp.find('.range-start').restoreDateFromData().css('opacity', 1).show(400);
                rp.find('.range-end').restoreDateFromData().css('opacity', 1).hide(400);
                setTimeout(function () { doneBtn.fadeIn(); }, 400);
                pickerOption = 1;
            }
