...
'
'$$Archive: /commonWebProject/DisplayCompanyDetail.aspx.vb $
'$$Author: Amanda $
'$$Date: 7/06/20 4:16p $
'$$Modtime: 7/06/20 4:16p $
'$$Revision: 28 $
'$$Workfile: DisplayCompanyDetail.aspx.vb $
'
' ********************************************************************************

Partial Public Class DisplayCompanyDetail
            ...
            Else
                ...
                If Not IsNothing(Request.Item("compid")) Then
                    If Not String.IsNullOrEmpty(Request.Item("compid").ToString) Then
                        ...
                        newWindow.Text = "<a href=""/DisplayEventsDetailListing.aspx?CompanyID=" + CompanyID.ToString + """,""EventsDetail"");"" target=""_blank"" title=""Show Events In New Window""  class=""float_right"">New Window</a>"
                    End If
                End If
                ...
                If clsGeneral.clsGeneral.isCrmDisplayMode() Then
                    ...
                    notesMainContainer.Visible = True
                ...
                If (Session.Item("jetnetAppVersion") = Constants.ApplicationVariable.CUSTOMER_CENTER) Or (Session.Item("jetnetAppVersion") = Constants.ApplicationVariable.HOMEBASE) Then
                    ...
                    Data_More_Link.Visible = True
                    Data_More_Link.Text = "<a href='DisplayCompanyDetail.aspx?compid=" & CompanyID & "&MoreData=Y'>Data Provider Summary</a>"   '<hr Class=""remove_margin"" />

                    Research_More_Link.Visible = True
                    Research_More_Link.Text = "<a href='DisplayCompanyDetail.aspx?compid=" & CompanyID & "&MoreResearch=Y'>Research Notes</a>"   '<hr Class=""remove_margin"" />
                Else
                    Data_More_Link.Visible = False
                    Research_More_Link.Visible = False
                End If

                Company_Flight_Link.Visible = True
                Company_Flight_Link.Text = "<a href='javascript:void();' href='#' onclick=""javascript:window.open('view_template.aspx?ViewID=28&ViewName=Operator/Airport Utilization&comp_id=" & CompanyID & "&aport_id=0');return false;"">Flights</a>"    ' 
        ...
        Try
            ...
            Else
                ...
                If AclsData_Temp.is_aerodex_insight() = True Then
                    ...
                    If Not IsNothing(Request("use_insight_roll")) Then
                        If Trim(Request("use_insight_roll")) = "Y" Then
                            Me.portfolio_link.Text = "<a href=""userPortfolio.aspx?comp_id=" & CRMJetnetID & "&use_insight_roll=Y"" target=""_blank"">Fleet Analysis</a>"
                        Else
                            Me.portfolio_link.Text = "<a href=""userPortfolio.aspx?comp_id=" & CRMJetnetID & """ target=""_blank"">Fleet Analysis</a>"
                        End If
                    Else
                        Me.portfolio_link.Text = "<a href=""userPortfolio.aspx?comp_id=" & CRMJetnetID & """ target=""_blank"">Fleet Analysis</a>"
                    End If
                    ...
                    If use_insight_op = True Then
                        Me.faa_data_link.Text &= "<Br/></br>UTILIZATION SUMMARIES BASED ON FLIGHT DATA&nbsp;<a href='/help/documents/589.pdf' target='_blank' class=""help_cursor""><img src='images/help-circle.svg' width='25' alt='View UTILIZATION SUMMARIES BASED ON FLIGHT DATA' border='0'/></a>"
                    ...
                    If CompanyID <> 0 Then
                        ...
                        Else
                            ...
                            If (Session.Item("jetnetAppVersion") = Constants.ApplicationVariable.CUSTOMER_CENTER) Or (Session.Item("jetnetAppVersion") = Constants.ApplicationVariable.HOMEBASE) Then
                                ...
                                ' set it to visible, in case there is any  - if there is none is will set to invisible inside the function 
                            ...
                            If Trim(Request("MoreData")) = "Y" Then
                                If (Session.Item("jetnetAppVersion") = Constants.ApplicationVariable.CUSTOMER_CENTER) Or (Session.Item("jetnetAppVersion") = Constants.ApplicationVariable.HOMEBASE) Then
                            ...
                            If Trim(Request("MoreResearch")) = "Y" Then
                                If (Session.Item("jetnetAppVersion") = Constants.ApplicationVariable.CUSTOMER_CENTER) Or (Session.Item("jetnetAppVersion") = Constants.ApplicationVariable.HOMEBASE) Then
                                    Fill_Research_Notes()
                                End If
                            End If
                            ...
                            If Not Page.IsPostBack Then
                                If JournalID > 0 Then
                                    ...
                                    If Not IsNothing(CheckTable) Then
                                        If CheckTable.Rows.Count > 0 Then
                                            ...
                                            history_toggle_buttons.Text = "<a " & DisplayFunctions.WriteDetailsLink(0, CompanyID, 0, 0, False, "", "", "") & ">View Current Company</a>"
                                        Else
                                ...
                                notesMainContainer.Visible = False
                                ...
                                If ((Not Session.Item("localUser").crmDemoUserFlag) And (Session.Item("localUser").crmEnableNotes) _
                                    ...
                                    Dim ProspectsLinkText As String = "javascript:load('edit_note.aspx?action=new&type=prospect&cat_key=0&from=companydetails&comp_ID=" & CompanyID & "&source=" & IIf(CRMSource = "CLIENT", "CLIENT", "JETNET") & "','unloaded_me','scrollbars=yes,menubar=no,height=435,width=1060,resizable=yes,toolbar=no,location=no,status=no');"
                                    ...
                                    If Trim(Request("source")) = "CLIENT" Then
                                        DisplayFunctions.DisplayLocalItems(masterPage.aclsData_Temp, 0, CompanyID, 0, Nothing, Nothing, True, False, False, False, 0, False, True, "CLIENT", prospects_label, False, False, 0, IIf(prospects_drops.SelectedValue = "O", True, False))
                                    Else
                                        DisplayFunctions.DisplayLocalItems(masterPage.aclsData_Temp, 0, CompanyID, 0, Nothing, Nothing, True, False, False, False, 0, False, True, "JETNET", prospects_label, False, False, 0, IIf(prospects_drops.SelectedValue = "O", True, False))
                                    End If
                                    'End If
                                    ...
                                    Dim NotesLinkText As String = DisplayFunctions.WriteNotesRemindersLinks(0, 0, CompanyID, 0, False, "&n=1", "Add Note")
                                    Dim ActionsLinkText As String = DisplayFunctions.WriteNotesRemindersLinks(0, 0, CompanyID, 0, False, "", "Add Action")

                                    notes_add_new.Text = "<a href=" & NotesLinkText & " class='special'>NOTES +</a>"

                                    action_add_new.Text = "<a href=" & ActionsLinkText & " class='special'>ACTIONS +</a>"

                                    If Trim(Request("source")) = "CLIENT" Then '
                                        notes_add_new.Text = "<p align='right'>" & DisplayFunctions.ViewAllNotesLink(CompanyID, 0, CRMSource, "padding_left") & " + " & DisplayFunctions.WriteNotesRemindersLinks(0, 0, CompanyID, 0, True, "&n=1&source=CLIENT", "Add Note") & "</p>"
                                        action_add_new.Text = "<p align='right'>+ " & DisplayFunctions.WriteNotesRemindersLinks(0, 0, CompanyID, 0, True, "&source=CLIENT", "Add Action") & "</p>"
                                    Else
                                        notes_add_new.Text = "<p align='right'>" & DisplayFunctions.ViewAllNotesLink(CompanyID, 0, CRMSource, "padding_left") & " + " & DisplayFunctions.WriteNotesRemindersLinks(0, 0, CompanyID, 0, True, "&n=1", "Add Note") & "</p>"
                                        action_add_new.Text = "<p align='right'>+ " & DisplayFunctions.WriteNotesRemindersLinks(0, 0, CompanyID, 0, True, "", "Add Action") & "</p>"
                                    End If
                                ...
                                ElseIf CRMView = True Then
                                    ...
                                    notes_add_new.Text = "<p align='right'>+ <a href=""javascript:void(0);"" class=""float_right"" onclick=""" & NotesLinkText & """>Add Note</a>" & DisplayFunctions.ViewAllNotesLink(CompanyID, 0, CRMSource, "padded_right float_left") & "</p>"
                                ...
                                If (Session.Item("jetnetAppVersion") = Constants.ApplicationVariable.CUSTOMER_CENTER) Or (Session.Item("jetnetAppVersion") = Constants.ApplicationVariable.HOMEBASE) Then
                                    ...
                                    Dim aTempTable As New DataTable
                                    If prospects_drops.SelectedIndex = 0 Then
                                        aTempTable = AclsData_Temp.Notes_Search_For_Prospect_View_Homebase("", "", "", "B','O", 0, "", "", "", "", "", "", 0, 0, "", False, False, "'Active'", "", 0, CompanyID, 0)
                                    ElseIf prospects_drops.SelectedIndex = 1 Then
                                        aTempTable = AclsData_Temp.Notes_Search_For_Prospect_View_Homebase("", "", "", "B','O", 0, "", "", "", "", "", "", 0, 0, "", False, False, "'Active','Closed','Inactive'", "", 0, CompanyID, 0)
                                    End If
    ...
    Private Sub SetUpTopMenuAddLinks()
        ...
        If CRMView = True Then
            ...
            If OtherID > 0 Then
                ...
                Else
                    edit_company_link.InnerHtml = CompanyFunctions.CreateCompanyEditLink(CRMSource, CRMView, CompanyID, True, False, True)
            ...
            ElseIf CRMSource <> "CLIENT" Then
                edit_company_link.InnerHtml = CompanyFunctions.CreateCompanyEditLink(CRMSource, CRMView, CompanyID, False, True, True)
                edit_company_link.Visible = True
            End If
        End If
    ...
    Private Sub Fill_Airport_Model_Tab()
        ...
        If use_insight_own = True Then
            ...
            aircraft_ownership_history_label.Text = Replace(aircraft_ownership_history_label.Text, "class=""Box""", "")
            this_section_string = ""
            If use_insight_roll = True Then
    ...
    Public Sub models_operated(ByVal holdtable As DataTable, ByRef string_return As String, ByVal main_comp_id As Long, ByVal amod_id As Long, ByVal localCriteria As viewSelectionCriteriaClass)
        ...
        If Not IsNothing(holdtable) Then
            If holdtable.Rows.Count > 0 Then
                htmlOut_Make.Append("<table id='modelForsaleViewOuterTable3' width=""100%"" cellpadding=""0"" cellspacing=""0""   class='formatTable blue'>")
    ...
    Public Sub Fill_Certifications_Tab(ByVal cert_category As String)
        ...
        If Not IsNothing(CertifactionsTable) Then
            If (CertifactionsTable.Rows.Count > 0) Then
                ...
                certifications_label.Text += "<img alt=""View Operator Certification Decriptions"" border=""0"" class=""float_right padding_left help_cursor"""
                certifications_label.Text += "src=""images/help-circle.svg"" width=""25"" /></a></div><br />"
    ...
    Public Sub Fill_Admin_Action_Items()
        ...
        If Not IsNothing(actionDataTable) Then
            action_add_new.Visible = True
            action_add_new.Text = "<a href='/adminActions.aspx?task=add&journid=&companyid=" & CompanyID & "&contactid=' target='_blank'>ADD NEW</a>"
        ...
        Else
            If masterPage.aclsData_Temp.class_error <> "" Then
                masterPage.LogError("DisplayCompanyDetail.aspx.vb -FillAdminActionItems() - " & masterPage.aclsData_Temp.class_error)
            End If
        End If
    End Sub
    ...
    Public Sub Fill_Trials_Summary_Tab()
        ...
        Dim htmlOut As New StringBuilder
        Dim temp_total_contact As Long = 0
        Dim last_service_name As String = ""
            ...
            If (service_table.Rows.Count > 0) Then
                ...
                For Each q As DataRow In service_table.Rows
                    ...
                    If Not IsDBNull(q("NAME")) Then
                        temp_name = Trim(q("NAME"))
                    End If

                    If (OldService <> q("SERVICE")) Or Trim(last_service_name) <> Trim(temp_name) Then    ' ADDED IN MSW -  3/30/20 so that if the service or person changes
    ...
    Public Sub Fill_Services_Used_Tab()
        ...
        add_services.Text = "<a href='/homeTables.aspx?type_of=Company&sub_type_of=ServicesUsed&comp_id=" & CompanyID.ToString & "' target='_blank'>ADD/UPDATE LIST</a>"
        If Not IsNothing(user_table) Then
    ...
    Public Sub Fill_Contacts_Tab()
        ...
        End If
        HttpContext.Current.Session.Item("localUser").crmUser_DebugText += "<br /> START Display_Contact_Details : " + Now.ToString + " <br />"
        ...
        HttpContext.Current.Session.Item("localUser").crmUser_DebugText += "<br /> END Display_Contact_Details : " + Now.ToString + "<br />"
...
#End Region
    ...
    Private Sub DisplayCompanyDetail_LoadComplete(sender As Object, e As EventArgs) Handles Me.LoadComplete
        ...
        '    toggleLink.Text = "<a href=""" & Replace(Request.Url.ToString().ToString, "Detail.aspx", "DetailClassic.aspx", 1, -1, CompareMethod.Text) & """>Classic View</a>"
        'Else
        '    toggleLink.Text = "<a href=""" & Replace(Request.Url.ToString().ToString, "DetailClassic.aspx", "Detail.aspx", 1, -1, CompareMethod.Text) & """>Tiled View</a>"
        'End If
        ...
        If Not Page.ClientScript.IsClientScriptBlockRegistered("masonryLoad") Then
            ...
            System.Web.UI.ScriptManager.RegisterStartupScript(Me, Me.GetType(), "masonryLoad", masonryStr, True)
        End If
        ...
        If Page.IsPostBack Then
            ...
            End If
        Else
            If Not Page.ClientScript.IsClientScriptBlockRegistered("masonryPostFolderCall") Then
                System.Web.UI.ScriptManager.RegisterStartupScript(Me, Me.GetType(), "masonryPostFolderCall", "$(window).on('load', function() {loadMasonry();});", True)
            End If
        End If
    End Sub

    Private Sub prospects_drops_SelectedIndexChanged(sender As Object, e As EventArgs) Handles prospects_drops.SelectedIndexChanged

        ' commented out MSW - 6/3/20
        'If (Session.Item("jetnetAppVersion") = Constants.ApplicationVariable.EVO) Then
        '    If ((Not Session.Item("localUser").crmDemoUserFlag) And (Session.Item("localUser").crmEnableNotes) _
        '                                                            And Session.Item("localSubscription").crmServerSideNotes_Flag And JournalID = 0) And (Not String.IsNullOrEmpty(Session.Item("jetnetServerNotesDatabase"))) Then
        ...
        If (Session.Item("jetnetAppVersion") = Constants.ApplicationVariable.CUSTOMER_CENTER) Or (Session.Item("jetnetAppVersion") = Constants.ApplicationVariable.HOMEBASE) Then
            ...
            '   AclsData_Temp.JETNET_DB = HttpContext.Current.Session.Item("jetnetAdminDatabase")
            AclsData_Temp.JETNET_DB = HttpContext.Current.Session.Item("jetnetClientDatabase")

            Dim aTempTable As New DataTable
            If prospects_drops.SelectedValue = "O" Then
                aTempTable = AclsData_Temp.Notes_Search_For_Prospect_View_Homebase("", "", "", "B','O", 0, "", "", "", "", "", "", 0, 0, "", False, False, "'Active'", "", 0, CompanyID, 0)
            Else
                aTempTable = AclsData_Temp.Notes_Search_For_Prospect_View_Homebase("", "", "", "B','O", 0, "", "", "", "", "", "", 0, 0, "", False, False, "'Active','Closed','Inactive'", "", 0, CompanyID, 0)
            End If
