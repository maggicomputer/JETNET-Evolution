...
'
'$$Archive: /commonWebProject/homebaseOpCosts.aspx.vb $
'$$Author: Mike $
'$$Date: 6/19/19 8:39a $
'$$Modtime: 6/18/19 6:12p $
'$$Revision: 2 $
'$$Workfile: homebaseOpCosts.aspx.vb $
'
' ********************************************************************************
...
Partial Public Class homebaseOpCosts
    ...
    If Not IsNothing(Request.Item("AmodID")) Then
      If Not String.IsNullOrEmpty(Request.Item("AmodID").ToString.Trim) Then
        If IsNumeric(Request.Item("AmodID").ToString) Then
          ...
          Master.SetPageTitle(make_model_name.Trim + " - Homebase Op Costs - " + WeekdayName(Weekday(Today)).ToString + ", " + MonthName(Month(Today)).ToString + " " + Day(Today).ToString + ", " + Year(Today).ToString)

          Build_OpCosts_Page(localCriteria)
  ...
  Private Sub Build_OpCosts_Page(ByRef searchCriteria As viewSelectionCriteriaClass)
    ...
    Try
      ...
      htmlOut.Append("<table id=""outerCostsTbl"" width=""100%"" cellpadding=""4"" cellspacing=""0"" border=""0"">")
      htmlOut.Append("<tr>")
      ...
      opCosts_listing_text.Text = htmlOut.ToString()

    Catch ex As Exception
      HttpContext.Current.Session.Item("localUser").crmUser_DebugText += "** Error in [homebaseOpCosts.aspx.vb :  [Build_OpCosts_Page] : " + ex.Message
    End Try
